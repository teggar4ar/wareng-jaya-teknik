...
## Performance traps (learned the hard way, 1 Aug 2026)

- **`Reveal` needs `priority` above the fold.** `src/components/ui/Reveal.jsx` wraps framer-motion, and `initial={{opacity:0}}` gets **serialised into the prerendered HTML** as `style="opacity:0"`. Without `priority`, above-the-fold content (including the `<h1>` wrapper and the hero `<img>`) is present in the HTML but invisible until framer-motion hydrates — this alone cost 5.2 s of LCP render delay on mobile. Any new above-the-fold `Reveal` must pass `priority`.
- **Fonts are self-hosted in `public/fonts/` with hand-written `@font-face` in `index.css`, deliberately not `@fontsource` imports.** The `@fontsource` CSS lists legacy `.woff` next to `.woff2`, and Vite emitted a high-priority `rel=preload` for *both* formats of all 9 faces: 18 requests / 403 KB competing with the LCP image. Current setup is woff2-only (174 KB) with exactly two preloads in `index.html` (Oswald 700, Inter 400). Don't reintroduce the `@fontsource` imports in `main.jsx`.
- **gtag.js is deferred to `requestIdleCallback` after `load`** (`index.html`), not `<script async>` in `<head>`, where it blocked the main thread ~280 ms. `dataLayer` + `gtag('config')` still run inline first so early events aren't lost.
- **`gtag('config')` sets `allow_google_signals: false` and `allow_ad_personalization_signals: false`.** With signals on, GA4 fires a remarketing pixel to `google.co.id/ads/ga-audiences` and a POST to `stats.g.doubleclick.net`, neither of which the CSP allows — two console errors on every page view. Widening `img-src`/`connect-src` to ad domains would silence them by collecting advertising data the site has no use for; turning the signals off is the correct fix. Analytics still reports normally (`/g/collect` → 204). Don't add ad domains to the CSP unless real ad campaigns are launched.
- Mobile Lighthouse after these fixes: performance 81 (was 55), LCP 3.7 s (was 6.2 s), TBT 230 ms (was 780 ms). Desktop 99. Remaining known failures: `color-contrast` (6 elements — the accent `#df500c` is 3.52:1 on paper, below AA; needs an owner decision on the token) and `charset` (helmet injects tags before `<meta charset>`, pushing it past the 1024-byte mark; harmless since Vercel sends the charset header).
