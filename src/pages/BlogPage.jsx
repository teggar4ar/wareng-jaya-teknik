              ...
              {tags.length > 0 && (
                <div className="mt-3">
                  <button 
                    ...
                    className={`text-sm flex items-center ${isDark ? 'text-blue-400 hover:text-blue-300' : 'text-blue-600 hover:text-blue-800'}`}
                  >
                    <span>{showTags ? 'Sembunyikan Tag' : 'Tampilkan Tag'} ({tags.length})</span>
                    <svg 
                      className={`w-4 h-4 ml-1 transition-transform ${showTags ? 'rotate-180' : ''}`} 
                      fill="none" 
                      stroke="currentColor" 
                      viewBox="0 0 24 24"
                    >
