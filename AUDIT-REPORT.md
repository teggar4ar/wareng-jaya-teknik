...
23. ✅ **Statistik fabrikasi, testimonial palsu, dan klaim sertifikasi dihapus.** Halaman di `src/pages/` sudah bersih sejak redesign Steelworks; sisanya semua ada di `content/blog/`. Yang dihapus: dua testimonial dengan nama & perumahan karangan (`bengkel-las-bojonggede.md`, "Bapak Hendra"/"Ibu Ratna"), tiga angka pengalaman yang **saling bertentangan** (15 tahun / 13 tahun / 10+ tahun di tiga artikel berbeda — tidak satu pun cocok dengan `/about` yang sengaja tidak menyebut angka), klaim "tenaga ahli bersertifikasi" + "sertifikasi keterampilan las", "material bersertifikat SNI", "puluhan tower air", garansi "hingga 5 tahun" (halaman lain hanya menjanjikan garansi sambungan las tanpa tenor), "jaminan kepuasan pelanggan", dan "powder coating dengan garansi warna". Frontmatter `description`/`excerpt` yang menjanjikan "garansi jangka panjang" dan "material premium" ikut diperbaiki karena masuk ke meta description. Boilerplate "tim profesional/tim ahli/tim teknisi" di 10 artikel diganti kalimat yang konsisten dengan posisi jujur di `/about` ("Anda bicara langsung dengan orang yang mengerjakan") — sebelumnya artikel menyiratkan struktur staf berjenjang yang tidak ada. Angka properti material (UV 98%, umur pakai 20–40 tahun, daur ulang 100%) **dipertahankan**: itu sifat bahan, bukan klaim tentang bisnis. `email: info@warengjayateknik.com` di JSON-LD `LocalBusiness` dihapus — alamat itu tidak dipakai di halaman mana pun dan tidak terverifikasi.
24. ✅ **Internal linking kontekstual.** Setiap artikel kini punya blok penutup yang menaut ke 2–3 artikel terkait plus halaman layanan/galeri/kontak yang relevan. Di arah sebaliknya, tiap `Service` di `/services` dapat field `article` yang menaut ke artikel pendukungnya (7 dari 8 layanan). Renderer `a` di `BlogPostPage.jsx` diubah: href yang dimulai `/` kini dirender sebagai `<Link>` router — sebelumnya **semua** link markdown dipaksa `target="_blank"`, jadi link internal membuka tab baru dan memicu reload penuh. Link eksternal tetap `_blank` + `noopener`.
25. ✅ **In-body link ke `/about`, `/blog`, `/contact`.** Ketiganya sebelumnya hanya menerima link dari header/footer global. Sekarang: Home menaut ke `/about` (paragraf hero) dan ke `/blog` + `/contact` (CTA akhir); `/services` menaut ke `/about` dan `/contact`; `/about` menaut ke `/services` dan `/contact`; `/gallery` dan `/contact` saling menaut ke `/services`. Terverifikasi di HTML prerender: `/services` 18 halaman perujuk, `/about` 15, `/blog` 15, `/contact` 8, `/gallery` 8 (sebelumnya 0 di dalam `<main>`).
26. ✅ **Focus management saat pindah route.** `ScrollToTop.jsx` diganti `RouteChangeHandler.jsx`: selain reset scroll, fokus keyboard dipindahkan ke `<h1>` halaman baru (`tabindex="-1"` disetel saat runtime, jadi tidak masuk tab order dan tidak muncul di HTML prerender) dan nama halaman diumumkan lewat live region `aria-live="polite"`. Render awal dikecualikan lewat ref — pada page load biasa browser dan screen reader sudah menangani sendiri. Semua akses DOM ada di dalam `useEffect`, jadi aman untuk prerender.
27. ⏳ **Verifikasi Core Web Vitals lapangan — masih belum bisa.** PageSpeed Insights API tetap mengembalikan HTTP 429 pada dua percobaan baru (tanpa API key). Perlu dicek manual lewat Search Console → Core Web Vitals, atau psi.web.dev, setelah deploy.

Verifikasi Fase 4: `npm run lint` bersih, `npm run build` berhasil memprerender 21 halaman. Grep di `dist/` mengonfirmasi nol sisa istilah fabrikasi (nama testimonial, angka tahun, klaim sertifikasi, email tak terpakai). Ke-17 target link kontekstual terkonfirmasi mengarah ke file HTML yang benar-benar ada (tidak ada broken link), dan nol link internal yang masih memakai `target="_blank"`.
